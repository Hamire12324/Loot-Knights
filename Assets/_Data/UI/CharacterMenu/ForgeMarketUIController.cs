...
public class ForgeMarketUIController : MonoBehaviour
{
    ...
    {
        SetText(goldText, PlayerCurrencyStorage.Coins.ToString("N0"));
        SetText(diamondText, PlayerCurrencyStorage.Diamonds.ToString("N0"));
    ...
    private void RefreshShopButtons()
    {
        ...
        if (bagGoldButton != null)
            bagGoldButton.interactable = PlayerCurrencyStorage.Coins >= goldCost;

        if (bagDiamondButton != null)
            bagDiamondButton.interactable = PlayerCurrencyStorage.Diamonds >= diamondCost;

        if (diamondExchangeButton != null)
            diamondExchangeButton.interactable = PlayerCurrencyStorage.Diamonds >= 1;
    }
